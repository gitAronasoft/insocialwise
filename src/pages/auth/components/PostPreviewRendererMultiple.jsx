...
const PostPreviewRenderer = ({ platform, post }) => {
    ...
    const renderFacebookPreview = (postData) => (
        <div className="preview-container-postlist facebook">
            <div style={{ display: 'flex', alignItems: 'center' }}>
                <div style={{ position: 'relative', display: 'inline-block', marginRight: 10 }}>
                    <img src={postData.pagePicture || `${process.env.PUBLIC_URL}/assets/images/avtar/user.png`} alt="Page"
                        style={{ width: 36, height: 36, borderRadius: '50%', marginRight: 10 }}
                        onError={(e) => { e.target.src = `${process.env.PUBLIC_URL}/assets/images/placeholder_img.jpg`; }}
                    />
                    <div title={postData.platform} style={{ position: 'absolute', bottom: '-4px', right: '-4px', width: '25px',
                        height: '25px', borderRadius: '50%', background: getPlatformColor(platform), display: 'flex',
                        alignItems: 'center', justifyContent: 'center', boxShadow: '0 0 2px rgba(0,0,0,0.3)', padding:'5px'  }}
                    >
                        {platformSVGs[postData.platform] || (
                ...
                <div>
                    <div style={{ fontWeight: 600 }}>{postData.pageName}</div>
            ...
            {(() => {
                try {
                    if (typeof postData.postMedia === 'string') {
                        if (postData.postMedia.startsWith('https://')) {
                            return <img src={postData.postMedia} alt="Post Media" className="post-image" 
                                    onError={(e) => { e.target.src = `${process.env.PUBLIC_URL}/assets/images/placeholder_img.jpg`; }} />;
                        } else {
                            return renderMediaPreview(postData.platform, postData.postMedia);
                        }
                    }
                } catch (err) {
    ...
    const renderLinkedInPreview = (postData) => (
            ...
            <div style={{ display: 'flex', alignItems: 'center' }}>
                <div style={{ position: 'relative', display: 'inline-block', marginRight: 10 }}>
                    <img src={postData.pagePicture || `${process.env.PUBLIC_URL}/assets/images/avtar/user.png`} alt="Page"
                        ...
                        onError={(e) => { e.target.src = `${process.env.PUBLIC_URL}/assets/images/placeholder_img.jpg`; }}
                    />
                    <div title={postData.platform} style={{ position: 'absolute', bottom: '-4px', right: '-4px', width: '25px', height: '25px',
                        borderRadius: '50%', background: getPlatformColor(postData.platform), display: 'flex', alignItems: 'center', justifyContent: 'center',
    ...
    const renderInstagramPreview = (postData) => (
    ...
    const renderPreview = () => {
        ...
        if (post.length === 0) {
            return (
                <div className="preview-container-postlist default">
                    <p>Preview not available for now.</p>
                </div>
            );
        }
        ...
        return (
            <>
                {post.posts.map((postData, index) => {
                    const platform = postData.platform?.toLowerCase();
                    // console.log("platform",platform);
                    let previewContent;
                    switch (platform) {
                        case "facebook":
                            previewContent = renderFacebookPreview(postData);
                            break;
                        case "instagram":
                            previewContent = renderInstagramPreview(postData);
                            break;
                        case "linkedin":
                            previewContent = renderLinkedInPreview(postData);
                            break;
                        default:
                            previewContent = (
                                <div className="preview-container-postlist default">
                    ...
                    return (
                        <div key={postData.id || index} className="post-preview-wrapper m-2">
                            {previewContent}
                        </div>
                    );
                })}
